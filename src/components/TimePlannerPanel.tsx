...
const STORAGE_KEY = 'time-planner-data';
const CATEGORIES_STORAGE_KEY = 'time-planner-categories';

interface TimePlannerPanelProps {
...
export default function TimePlannerPanel({ selectedDate, onReset, isCollapsed = false, onToggleCollapse }: TimePlannerPanelProps) {
  ...
  useEffect(() => {
    console.log('🔄 TimePlannerPanel useEffect triggered:', { selectedDate, isAuthenticated, hasUser: !!user, userId: user?.uid, hasLoaded: hasLoadedRef.current[selectedDate], previousDate: previousDateRef.current, isLoading: isLoadingRef.current });
    ...
    const loadData = async () => {
      ...
      if (isAuthenticated && user) {
        console.log('🔍 Attempting to load time planner data from Firebase for:', selectedDate, 'user:', user.uid);
        try {
          ...
          if (firebaseData) {
            console.log('📥 Loaded time planner data from Firebase:', selectedDate, 'blocks:', firebaseData.blocks?.length || 0, 'data:', firebaseData);
            // Firebase 데이터가 있으면 무조건 우선 사용 (동기화 우선)
            // Firebase 데이터가 있으면 항상 사용 (blocks가 없어도 구조는 유지)
            dayData = firebaseData;
            console.log('✅ Using Firebase data for time planner:', selectedDate, 'blocks count:', dayData.blocks?.length || 0);
          } else {
            console.log('ℹ️ No Firebase data found for time planner:', selectedDate, 'using localStorage');
          }
          
          // 카테고리도 Firebase에서 로드 (전역 설정)
          try {
            const firebaseCategories = await timePlannerService.getTimePlannerCategories();
      ...
      // 3. 데이터 적용
      console.log('🔍 Applying time planner data for:', selectedDate, 'dayData:', dayData, 'blocks:', dayData?.blocks?.length || 0);
      
      if (dayData && dayData.blocks && Array.isArray(dayData.blocks)) {
        const loadedBlocks = dayData.blocks;
        console.log('📦 Time planner blocks to apply:', loadedBlocks.length, 'blocks:', loadedBlocks);
        ...
        if (isValidBlocks) {
          console.log('✅ Setting time planner blocks:', loadedBlocks.length);
    ...
    try {
      ...
      if (isAuthenticated && user) {
        try {
          ...
          console.log('✅ Time planner data saved to Firebase:', dataToSave.date, 'blocks:', blocksToSave.length);
        } catch (error) {
          console.error('❌ Failed to save time planner data to Firebase:', error);
        }
      }
    } catch (error) {
      // 저장 실패 시 무시
  ...
  if (isCollapsed) {
    return (
      <div className="time-planner-wrapper">
